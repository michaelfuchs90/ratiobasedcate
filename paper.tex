...
A difference-based effect of $\tau_{\text{diff}}(x) = 0.05$ represents
a sixfold increase when the baseline is $\mu_0(x) = 0.01$ but merely
a 25\% lift when $\mu_0(x) = 0.2$. \citet{schmid1998} give
corresponding empirical evidence: control-group event rates predict
absolute but not relative treatment effects across clinical trials,
suggesting that the ratio scale better captures the intrinsic treatment
mechanism. Many domains of interest fall naturally in this regime,
with baseline rates often between 1--20\% in clinical trials and
1--10\% in marketing campaigns.
...
heterogeneity, or apply generic outcome regression (S- or T-Learner
style) without exploiting the multiplicative structure of the ratio
functional and without robustness guarantees against model
misspecification.
...
separate log-outcome models, and no doubly robust estimator for the heterogeneous ratio CATE exists. More generally, robustness
