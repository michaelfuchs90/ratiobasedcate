...
The most striking result is the failure of DR-Q (mean Qini ratio
$0.30$, $\sigma_D = 0.69$, the highest in the panel): it produces
\emph{negative} Qini on both RHC ($-0.106$) and NHEFS ($-0.082$),
meaning worse-than-random targeting. This confirms that
DR-Q's conditional robustness requires exact propensity knowledge
(Section~\ref{res:dr-q-direct}), which is unavailable in observational
settings. DR-Q log (mean ratio $0.60$) fares substantially better due
to variance stabilisation, but still lags behind DR-S/T log.
...
\begin{enumerate}
    ...
    is the decisive ingredient. DR-Q collapses entirely in this setting,
    producing negative Qini on two datasets, confirming that its
    conditional robustness requires exact propensity knowledge unavailable
...
both necessary when propensity must be estimated. DR-Q's conditional
robustness proves insufficient under confounding, producing
worse-than-random targeting on two datasets. Across both settings, the
