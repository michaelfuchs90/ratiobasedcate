...
learner dominates, but the Q-Learner is the most consistently competitive
across datasets. On five observational datasets, where propensity must
be estimated and confounding cannot be ruled out, the log-scale DR
learners introduced here are clearly superior.
\end{abstract}
...
learner dominates, but the Q-Learner is the most consistently competitive
ranker across datasets, with the S-Learner a close second on the mean but
markedly less stable. In observational settings, where propensity must be
estimated and confounding cannot be ruled out, the S/T log-scale DR
learners introduced here are clearly superior for ranking, confirming the
importance of classical double robustness in such settings. Across both
regimes, the S-Learner is the best-calibrated method.
...
finite-sample variance. Fitting $\hat{e}(x)$ from data---even though the
model only captures the randomness of the treatment assignments---preserves
this correlation and thereby reduces variance.
...
Qini). For applications where computational efficiency matters,
Q-Simple remains a competitive choice on most RCTs.
...
conditions to give guidance on when to use which. Further, the theoretical
analysis in Section~\ref{sec:dr} leads to three predictions we want to
test. First, in observational settings DR-S and DR-T should outperform
DR-Q: their classical double robustness tolerates propensity
misspecification, while DR-Q's conditional robustness does not. Second,
we expect the simple variants of the Q-Learner family to be slightly
worse than their non-simple counterparts, but by how much is an open
question. Third, log-scale DR variants should be more stable than their
direct-scale counterparts when treatment effects vary substantially in
magnitude.
...
T-Learner, DR-Q-Simple, and DR-T log). In the cross-dataset
ratio-to-best summary (Figure~\ref{fig:qini-gap}, left panel), the
\textbf{Q-Learner is the most consistently competitive method}: it
attains the highest mean Qini ratio across datasets ($0.77$) and the
highest worst-case ratio ($R_{\min}^Q = 0.40$). The S-Learner is a
close second on the mean ($0.75$) but markedly less stable, with a
worst-case ratio of $0.30$ on Lenta.

The broader pattern on RCT data is that \textbf{plug-in learners
suffice}: the top two positions by mean Qini ratio are held by plug-ins
(Q: $0.77$, S: $0.75$), while the best DR variant (DR-Q-Simple)
reaches $0.67$ and every other DR variant falls below $0.63$. On the
four RCTs where a DR variant is the outright winner (H(Vis), H(Conv),
X5, Twins), the best plug-in attains between $84\%$ and $96\%$ of the
DR winner's Qini, so even in the cases where DR wins, plug-ins follow
very closely. This matches the theoretical expectation: when
propensity is known by design and no confounding can arise, plug-in
learners can correctly identify the CATE. The DR augmentation then
trades a small bias-correction benefit against added estimation
variance from the nuisance correction term. While a DR learner
sometimes still comes out on top, our results suggest that the
additional variance often dominates the bias reduction.


\paragraph{Q-Learner vs.\ Q-Simple.}
The Q-Learner (mean Qini ratio $0.77$) outperforms Q-Simple ($0.59$),
and on the raw Qini scale the advantage is moderate (mean $0.39$ vs.\
$0.36$). The gap is concentrated on two datasets: on Lenta, Q-Simple's
Qini collapses to $0.002$ while the Q-Learner reaches $0.089$, and on
...
of each other. Q-Simple therefore remains a competitive, computationally
cheaper alternative on most RCTs, but the Q-Learner is the safer default.
...
($-0.082$), with a mean ratio of $0.30$ and a worst-case ratio of
$R_{\min}^Q = -0.55$ on RHC---the largest failure across all learners
in the panel. However, attributing this entirely to conditional robustness
would be misleading: the direct scale introduces additional variance that
amplifies the problem. The cleaner test is DR-Q log (mean ratio $0.59$),
...
vs.\ DR-S $0.57$; DR-T log $0.75$ vs.\ DR-T $0.58$; DR-Q log $0.59$
...
\paragraph{Stability of the plug-in Q-Learner.}
...
negative on at least one dataset (DR-Q $-0.55$, DR-T $-0.20$, DR-S
$-0.14$), and even the log-scale DR variants drop below $0.17$ on
their weakest dataset. Combined with its $R_{\min}^Q = 0.40$ on the
RCT panel, the Q-Learner is the only learner that never produces a
per-dataset failure across the twelve datasets.
...
(mean ratio $1.02$, $R_{\max}^C = 1.08$), followed by DR-S log
($1.10$) and DR-S ($1.20$). DR learners on direct scale remain
...
on every single observational dataset (mean ratio $1.00$,
$R_{\max}^C = 1.00$). The log-scale DR variants that dominate ranking
...
\begin{enumerate}
    ...
    learner dominates, but the Q-Learner is the most consistently
    competitive method, with the S-Learner a close second on the mean
    but less stable across datasets; every DR variant lags behind. Even
    on datasets where a DR variant wins, the best plug-in follows
...
learners prove sufficient: the Q-Learner is the most consistently
competitive method, with the S-Learner a close second on the mean but
less stable across datasets, and every DR variant lags behind. Even on
the four RCT datasets where a DR variant wins, the best plug-in follows
...
These results suggest a simple decision rule: use the Q-Learner as
the default on RCTs (with the S-Learner as an interchangeable
alternative on most datasets), falling back to Q-Simple when
computational efficiency matters; use DR-S log when ranking is the
...
\hline
S
 & \textbf{1.224} & 2.316 & \textbf{1.028} & 1.076 & \textbf{1.024} & \textbf{1.113} & 1.397 & \textbf{1.311} & 1.084 \\
...
\hline
S
 & \textbf{1.573} & -- & \textbf{2.156} & \textbf{1.853} & \textbf{1.191} & \textbf{1.693} & 1.000 \\
 & \scriptsize(0.019) & \scriptsize(--) & \scriptsize(0.042) & \scriptsize(0.034) & \scriptsize(0.005) & & \\[2pt]
T
 & 1.648 & -- & 6.293 & 3.931 & 1.362 & 3.308 & 2.919 \\
...
DR-S
 & 1.764 & \textbf{1.126} & 2.708 & 2.303 & 1.389 & 1.858 & 1.256 \\
...
\hline
\textit{Dataset mean} & 1.848 & 1.330 & 3.998 & 4.104 & 1.401 & & \\
\hline
\end{tabular}
\end{table}
...
\hline
S-Learner & 1.47$\times$ & 2.05$\times$ & $+$40\% \\
T-Learner & 2.23$\times$ & 4.84$\times$ & $+$117\% \\
